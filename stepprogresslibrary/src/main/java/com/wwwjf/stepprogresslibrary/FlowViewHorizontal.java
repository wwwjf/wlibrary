...
//    private Map<String, String> map;
    ...
    public FlowViewHorizontal(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        bgRadius = ta.getDimension(R.styleable.FlowViewHorizontal_h_bg_radius, 16);
        proRadius = ta.getDimension(R.styleable.FlowViewHorizontal_h_pro_radius, 16);
        lineBgWidth = (int) ta.getDimension(R.styleable.FlowViewHorizontal_h_bg_width, 3f);
        bgColor = ta.getColor(R.styleable.FlowViewHorizontal_h_bg_color, Color.parseColor("#e8eaf2"));
        lineProWidth = (int) ta.getDimension(R.styleable.FlowViewHorizontal_h_pro_width, 2f);
        proColor = ta.getColor(R.styleable.FlowViewHorizontal_h_pro_color, Color.parseColor("#3b55e6"));
    ...
    private void init() {
        ...
        hookPaint = new Paint();
        hookPaint.setStrokeWidth(lineProWidth >> 1);
        hookPaint.setAntiAlias(true);
        hookPaint.setColor(Color.WHITE);
        hookPaint.setStyle(Paint.Style.FILL);
    ...
    private void drawText(Canvas canvas) {
        for (int i = 0; i < maxStep; i++) {
            if (null != times && i < times.length)
                canvas.drawText(times[i], startX + (i * interval), bgCenterY + timePadding, textPaint);
        }
    }
